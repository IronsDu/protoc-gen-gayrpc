...
{{end}}
    ...
    public:
        ...
        {{end}}
        {{range $i, $method := $service.Methods}}ananas::Future<std::pair<{{$.ContainerNamespace}}{{$method.OutputType}}, gayrpc::core::RpcError>> Sync{{$method.MethodName}}(
            const {{$.ContainerNamespace}}{{$method.InputType}}& request,
            std::chrono::seconds timeout)
        {
            ananas::Promise<std::pair<{{$.ContainerNamespace}}{{$method.OutputType}}, gayrpc::core::RpcError>> promise;
    ...
    public:
        ...
        {
            return u8"{{$.ContainerNamespace}}{{$service.Name}}";
        }
    ...
    inline bool {{$service.Name}}Service::Install(const {{$service.Name}}Service::PTR& service)
    {
        ...
        {{$service.Name}}ServiceHandlerMapByStr serviceHandlerMapByStr = {
            {{range $i, $method := $service.Methods}}{u8"{{range $i, $packageName := $.PackageNames}}{{$packageName}}.{{end}}{{$service.Name}}.{{$method.MethodName}}", {{$service.Name}}Service::{{$method.MethodName}}_stub},
            {{end}}
        };
