...
{{end}}
    ...
    public:
        ...
        typedef std::weak_ptr<{{$service.Name}}Client> WeakPtr;
    ...
    public:
        ...
        {{range $i, $method := $service.Methods}}{{$.ContainerNamespace}}{{$method.OutputType}} Sync{{$method.MethodName}}(const {{$.ContainerNamespace}}{{$method.InputType}}& request,
    ...
    public:
        ...
        typedef std::weak_ptr<{{$service.Name}}Service> WeakPtr;
    ...
    private:
            ...
            const UnaryServerInterceptor& outboundInterceptor)
        {
            ...
            case RpcMeta::BINARY:
                ...
                {
                    throw std::runtime_error("parse binary {{$method.Name}} error");
                }
                break;
            case RpcMeta::JSON:
                {
                    ...
                    if (!s.ok())
                    {
                        throw std::runtime_error("parse json {{$method.Name}} failed:" +
                            s.error_message().as_string());
                    }
                }
                break;
            default:
                throw std::runtime_error("parse {{$method.Name}} of unsupported encoding type:" + meta.encoding());
            }
    ...
    inline bool {{$service.Name}}Service::Install(gayrpc::core::RpcTypeHandleManager::PTR rpcTypeHandleManager,
        const {{$service.Name}}Service::PTR& service,
        const UnaryServerInterceptor& inboundInterceptor,
        const UnaryServerInterceptor& outboundInterceptor)
    {
